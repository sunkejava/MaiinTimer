...
namespace BridImage.Utils
{
    public static class LoadResourceDll
    {
        ...
        /// 释放dll资源
        /// </summary>
        public static void ReleaseDll(string pattern = "*.dll")
        {
            System.IO.Directory.GetFiles("", "");
            //获取调用者的程序集
            var ass = new StackTrace(0).GetFrame(1).GetMethod().Module.Assembly;
            ...
            foreach (var r in res)
            {
                ...
                if (regex.IsMatch(r))
                {
                    try
                    {
                        var s = ass.GetManifestResourceStream(r);
                        var bts = new byte[s.Length];
                        if (r.Replace("CSkinDemo.DLL.", "") == "CSkin.dll" || r.Replace("CSkinDemo.DLL.", "") == "OMCS.dll")
                        {
                            s.Read(bts, 0, (int)s.Length);
                            var da = Assembly.Load(bts);
                            //判断是否已经加载
                        ...
                        {
                            string strPath = Application.StartupPath + @"\" + r;//设置释放路径                                          //创建dll文件（覆盖模式）
                    ...
                    {
                        MessageBox.Show("error:释放" + r + "文件失败,原因为：" + ex.Message);
                    }
                }
            }
        }
