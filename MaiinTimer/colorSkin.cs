    ...
    public partial class colorSkin : LayeredForm
    {
        ...
        public void Init()
        {
            ...
            for (int i = 1; i <= 24; i++)
            {
                ...
                dlc.Width = (Width - 10) / 8;
                dlc.Height = (Height - 5) / 3;
                dlc.Name = "color" + i.ToString();
                //根据循环数判断当前行及列
                int ColorColNum = (i % 8 == 0 ? 8 : i % 8);
                int ColorRowNum = (i - i % 8) / 8 + (i % 8 == 0 ? 0 : 1);
                dlc.Location = new Point(3 + ((ColorColNum - 1) * dlc.Width), 3 * (ColorRowNum) + ((ColorRowNum - 1) * dlc.Height));
